...
68:function(objekt, rad, col) {
  ...
  floorCol(objekt, rad) {
   if(objekt.y - objekt.old_y > 0){
    ...
    var bunn = rad * 32;
       ...
       if ((objekt.y +32)> bunn  && (objekt.old_y + 32) <= bunn) {
  ...
  rightCol(objekt, col){
      if(objekt.x-objekt.old_x > 0) {
        var vegR = col* 32;
        if((objekt.x+32)>vegR && (objekt.old_x+32) <= vegR){
            objekt.speedX = 0;
...
function colLoop(){
    ...
    if(this.spilleren.y - this.spilleren.old_y > 0){
        ...
        var bunn_rad = Math.floor((this.spilleren.y+32)/32);
        ...
        var right_col = Math.floor((this.spilleren.x+32)/32);
